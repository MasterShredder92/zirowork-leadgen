window.PortalMyBusiness = function PortalMyBusiness({ tenantId }) {
  ...
  React.useEffect(() => {
    async function load() {
      ...
      const [clientRes, tenantRes] = await Promise.all([
        ...
        window.sb.from('agent_tenants').select('*').eq('tenant_id', tenantId).single(),
      ]);
  ...
  function addTestimonial() {
    setForm(f => ({ ...f, testimonials: [...f.testimonials, ''] }));
  }
  ...
  async function handleSave() {
    ...
    const np = v => (v === '' || v === null || v === undefined ? null : Number(v));
    ...
    const [clientRes, tenantRes] = await Promise.all([
      window.sb.from('clients').update(clientUpdate).eq('id', tenantId),
      window.sb
        .from('agent_tenants')
  ...
  const field = (label, key, props = {}) => (
    <div style={s.fieldWrap}>
      ...
      <input style={s.input} value={form[key]} onChange={e => set(key, e.target.value)} {...props} />
    </div>
  );
  ...
  return (
    <div style={s.page}>
      ...
      <div style={s.chipRow}>
        <div style={s.chip(data.tenant.status === 'active')}>● Active</div>
        <div style={s.chip(false)}>{data.tenant.plan_tier || 'individual'} plan</div>
      </div>
      ...
      <div style={s.section}>
        ...
        <div style={s.chipRow}>
          {instruments.length
            ? instruments.map(inst => <div key={inst} style={s.chip(true)}>{inst}</div>)
            : <div style={s.chip(false)}>No instruments on file</div>}
        </div>
