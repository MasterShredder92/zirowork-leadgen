window.PortalBilling = function PortalBilling({ tenantId }) {
  ...
  const [setup, setSetup] = React.useState(null); // { client_secret, publishable_key }
  ...
  const stripeRef = React.useRef(null); // { stripe, card }
  ...
  function loadStripeJs() {
    return new Promise((resolve, reject) => {
      ...
      let el = document.getElementById('zw-stripe-js');
      if (!el) {
      ...
      el.addEventListener('error', () => reject(new Error('Could not load Stripe.js')));
    });
  }
  ...
  async function handleAddCard() {
    setCardError('');
    if (tenantId === 'preview') {
      setCardError('Card setup is disabled in preview mode.');
      return;
    }
    setStartingSetup(true);
    try {
      await loadStripeJs();
      const data = await callBilling({ action: 'setup-intent', tenant_id: tenantId });
      setSetup(data);
    } catch (e) {
  ...
  // Mount the Stripe card element once the setup form is on screen
  React.useEffect(() => {
    if (!setup || !cardDivRef.current || !window.Stripe) return;
    const stripe = window.Stripe(setup.publishable_key);
    const card = stripe.elements().create('card', { style: { base: { fontSize: '14px' } } });
    card.mount(cardDivRef.current);
    stripeRef.current = { stripe, card };
    return () => { card.destroy(); stripeRef.current = null; };
  }, [setup]);
  ...
  async function handleSaveCard() {
    ...
    try {
      ...
      await callBilling({ action: 'confirm-pm', tenant_id: tenantId, payment_method: result.setupIntent.payment_method });
      setSetup(null);
      setLoading(true);
      await loadStatus();
    } catch (e) {
        ...
        {setup ? (
          <div>
            ...
            <div style={{ display: 'flex', gap: 10 }}>
              ...
              <button style={s.ghostBtn} onClick={() => { setSetup(null); setCardError(''); }}>Cancel</button>
            </div>
          </div>
        ) : status.card ? (
          <div style={s.cardRow}>
