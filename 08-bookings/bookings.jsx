  ...
  return (
    <div style={{ height: '100%', display: 'flex', flexDirection: 'column', background: T.bg }}>
      ...
      <div style={{ padding: '20px 24px', borderBottom: `1px solid ${T.border}`, flexShrink: 0 }}>
        <h1 style={{ fontSize: 24, fontWeight: 700, color: T.t1, letterSpacing: '-0.4px', margin: '0 0 4px 0' }}>Bookings</h1>
        <div style={{ fontSize: 12, color: T.t3 }}>What has ZiroWork scheduled and is the lead enrolled?</div>
      </div>
          ...
          <thead>
            <tr>
              {[
                ...
                <th key={i} style={{ ...(i === 0 ? firstCell : i === arr.length - 1 ? lastCell : cell), color: T.t4, fontWeight: 700, fontSize: 10, textTransform: 'uppercase', letterSpacing: '0.07em', textAlign: h.align }}>{h.label}</th>
              ))}
            </tr>
          </thead>
          <tbody>
            {bookings.map(b => (
              <tr key={b.id}
                ...
                <td style={firstCell}>
                  ...
                  <div style={{ fontSize: 11, color: T.t4 }}>{b.student_name}</div>
                </td>
                <td style={cell}>{b.client_name}</td>
                <td style={cell}>
                  <span style={{ fontSize: 11, fontWeight: 600, color: programColor(b.program), background: programColor(b.program) + '1A', padding: '2px 7px', borderRadius: 20 }}>
                    {b.program}
                  </span>
                </td>
                <td style={cell}>
                  <span style={{ fontSize: 11, fontWeight: 600, color: statusColor(b.status), background: statusColor(b.status) + '1A', padding: '2px 8px', borderRadius: 20 }}>
                  ...
                  {b.status !== 'completed' && b.status !== 'no_show' && (
                    <div style={{ display: 'flex', gap: 6, justifyContent: 'flex-end' }}>
                      ...
                      <button onClick={() => markBooking(b.id, 'no_show')}
                        style={{ padding: '3px 10px', border: `1px solid ${T.border}`, borderRadius: 6, background: 'none', fontSize: 11, color: T.t3, cursor: 'pointer', fontFamily: "'Plus Jakarta Sans', sans-serif" }}>
                        No-Show
                      </button>
                    </div>
                  )}
