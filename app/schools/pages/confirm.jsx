...
function ConfirmPage({ school, client, slug }) {
  ...
  const badge = {
    ...
    borderRadius: 20, fontSize: 11, fontWeight: 700, letterSpacing: '0.08em',
    textTransform: 'uppercase', marginBottom: 28,
  };

  if (status === 'loading') {
    return <div style={wrap}><div style={{ color: '#aaa', fontSize: 14 }}>Loading your details…</div></div>;
  }
  ...
  if (status === 'notfound') {
    return (
      <div style={wrap}>
        ...
        <h1 style={{ fontSize: 26, fontWeight: 800, color: '#1a1a1a', margin: '0 0 12px' }}>Link not found</h1>
        <p style={{ fontSize: 15, color: '#888', maxWidth: 380, lineHeight: 1.6 }}>
  ...
  if (status === 'done' || status === 'already') {
    return (
      <div style={wrap}>
        ...
        <p style={{ fontSize: 17, color: '#555', fontWeight: 600, margin: '0 0 8px', maxWidth: 420 }}>
        ...
        <p style={{ fontSize: 14, color: '#aaa', maxWidth: 400, lineHeight: 1.6 }}>
  ...
  return (
    <div style={wrap}>
      ...
      <p style={{ fontSize: 15, color: '#888', margin: '0 0 28px', maxWidth: 400, lineHeight: 1.6 }}>
      ...
      <div style={{ background: '#f8f8f6', border: '1px solid #f0f0ee', borderRadius: 14, padding: '24px 28px', marginBottom: 28, minWidth: 280, maxWidth: 360, width: '100%' }}>
        <div style={{ fontSize: 11, fontWeight: 700, letterSpacing: '0.1em', textTransform: 'uppercase', color: '#aaa', marginBottom: 12 }}>Your lesson</div>
        <div style={{ fontSize: 20, fontWeight: 800, color: '#1a1a1a', marginBottom: 6 }}>{(booking.program || 'Music').split(',')[0]}</div>
        {(booking.date || booking.time) && (
          <div style={{ fontSize: 15, color: '#555', fontWeight: 600 }}>
            {prettyDate(booking.date)}{booking.date && booking.time ? ' · ' : ''}{booking.time || ''}
          </div>
        )}
        {booking.teacher && <div style={{ fontSize: 14, color: '#888', marginTop: 4 }}>with {booking.teacher}</div>}
      </div>
      ...
      <button
        ...
        style={{
          ...
          fontSize: 16, fontWeight: 800, cursor: status === 'confirming' ? 'not-allowed' : 'pointer',
      ...
      {err && <div style={{ marginTop: 16, fontSize: 13, color: '#ef4444', maxWidth: 360 }}>{err}</div>}
    </div>
  );
}
