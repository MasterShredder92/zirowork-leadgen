...
2. **`08-bookings` filter.** `useBookings({ client_id })` filters a column `bookings` doesn't have. Latent (no client filter UI yet). When a filter is added, attribute via `lead_id → leads.client_id`, not `client_id`.
3. **`02-onboarding` checklist.** Toggles write booleans into the `clients.sms_number` / `lead_form_webhook` **text** columns, clobbering real values. The onboarding-checklist booleans (`automation_rules`, `integrations`, `brand_assets`, `protected_slots`) are also **not** the rich `automation_rules` / `integrations` / `assets` TABLES — same name, unrelated meaning. Don't confuse them.
