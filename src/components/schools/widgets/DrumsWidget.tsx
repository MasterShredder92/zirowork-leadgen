...
export default function DrumsWidget({ accent }: { accent: string }) {
  ...
  const hit = (pad: { id: string; label: string; emoji: string; file: string; color: string }) => {
    setFlash(f => ({ ...f, [pad.id]: true }));
    setTimeout(() => setFlash(f => ({ ...f, [pad.id]: false })), 150);

    const audio = new Audio(String(AUDIO_BASE) + pad.file);
    audio.play().catch(() => {
      const ctx = new (window.AudioContext || (window as typeof window & { webkitAudioContext: typeof AudioContext }).webkitAudioContext)();
      const fb = FALLBACK_FREQS[pad.id] || { f: 200, type: 'sine' as OscillatorType, dur: 0.3 };
