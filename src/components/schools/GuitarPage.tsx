...
export default function GuitarPage({ school, intakeUrl }: { school: School; intakeUrl: string | null }) {
  ...
  return (
    <Fragment>
      ...
      <section style={{ padding: '64px 24px 8px', background: 'var(--color-school-bg-subtle)' }}>
      ...
      <section style={{ padding: '80px 24px', background: 'var(--color-school-bg-card)' }}>
        <div style={{ maxWidth: 720, margin: '0 auto', textAlign: 'center' }}>
          ...
          <p style={{ fontSize: 16, color: 'var(--color-school-text-6)', margin: 0, lineHeight: 1.75 }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
            {FAQS.map((f, i) => (
              <div key={i} style={{ background: 'var(--color-school-bg-card)', borderRadius: 12, border: '1.5px solid var(--color-school-border-2)', boxShadow: '0 2px 8px rgba(0,0,0,0.06)', overflow: 'hidden' }}>
                ...
                {openFaq === i && (
                  <div style={{ padding: '0 24px 20px', fontSize: 15, color: 'var(--color-school-text-3)', lineHeight: 1.75, borderTop: '1px solid var(--color-school-border-4)' }}>{f.a}</div>
                )}
              </div>
            ))}
          </div>
