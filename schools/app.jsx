...
const SUPABASE_ANON_KEY = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InR4cGd5dWV0ZnNyemZ4eG9wd3pmIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NzkyMDk5MzQsImV4cCI6MjA5NDc4NTkzNH0.LaSe5Gfho9WIqKQOyBECKHx4CbtIO95RexqoAQMkIvQ';
        ...
        <div style={{ maxWidth: 1100, margin: '0 auto', display: 'flex', alignItems: 'center', justifyContent: 'space-between', height: 64 }}>
          {school.logo
            ...
            : <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                <div style={{ width: 36, height: 36, borderRadius: 8, border: '2px dashed #ccc', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 11, color: '#bbb', fontWeight: 600, flexShrink: 0 }}>LOGO</div>
                <div style={{ fontSize: 17, fontWeight: 700, color: '#1a1a1a', letterSpacing: '-0.02em' }}>{school.name}</div>
              </div>
          }
          <div style={{ display: 'flex', gap: 20, alignItems: 'center' }}>
            {school.phone && (
              <a href={'tel:' + school.phone.replace(/\D/g, '')} style={{ fontSize: 14, color: '#555', textDecoration: 'none', display: 'none', fontWeight: 500 }}
                className="nav-phone">
                {school.phone}
              </a>
            )}
            <a href={intakeUrl} style={{ padding: '9px 22px', background: accent, color: '#fff', borderRadius: 8, fontSize: 14, fontWeight: 700, textDecoration: 'none', whiteSpace: 'nowrap', letterSpacing: '-0.01em' }}>
              Get Started →
            </a>
          </div>
        </div>
      ...
      <footer style={{ background: '#111', color: '#fff', padding: '20px 24px' }}>
        <div style={{ maxWidth: 1100, margin: '0 auto', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: '12px 24px' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 16, flexWrap: 'wrap' }}>
            <div style={{ fontSize: 14, fontWeight: 700, color: '#fff' }}>{school.name}</div>
            {school.phone && <a href={'tel:' + school.phone.replace(/\D/g, '')} style={{ fontSize: 13, color: '#666', textDecoration: 'none' }}>{school.phone}</a>}
            {school.city && <span style={{ fontSize: 13, color: '#555' }}>{school.city}{school.state ? ', ' + school.state : ''}</span>}
          </div>
