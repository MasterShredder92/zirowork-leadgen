  ...
  return (
    <div style={{ height: '100%', overflowY: 'auto', padding: '32px 40px' }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 28 }}>
        <div>
          <div style={{ fontSize: 22, fontWeight: 700, color: T.t1, letterSpacing: '-0.4px', marginBottom: 4 }}>Client Onboarding</div>
          <div style={{ fontSize: 13, color: T.t3 }}>What is missing before a client can launch?</div>
        </div>
        <button onClick={() => setWizardOpen(true)} style={{ padding: '9px 18px', background: T.accent, color: '#fff', border: 'none', borderRadius: 8, fontSize: 13, fontWeight: 600, cursor: 'pointer', fontFamily: "'Plus Jakarta Sans', sans-serif" }}>
          + New Client
        </button>
      </div>
            ...
            return (
              <div key={client.id} style={{ padding: '20px 24px', background: T.cardBg || 'var(--surface)', border: `1px solid ${T.border}`, borderRadius: 10 }}>
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 16 }}>
                  <div>
                    <div style={{ fontSize: 15, fontWeight: 600, color: T.t1, marginBottom: 2 }}>{client.name}</div>
                    <div style={{ fontSize: 12, color: T.t4 }}>{client.city}</div>
                  </div>
                  <div style={{ textAlign: 'right' }}>
                    <div style={{ fontSize: 20, fontWeight: 700, color: pct === 100 ? '#22C55E' : T.accent }}>{pct}%</div>
